...
esp_websocket_client_config_t websocket_cfg = {
    .uri = "ws://127.0.0.1:5500", // <-- Replace with your WebSocket server URI
};
    ...
    switch (event_id) {
        case WEBSOCKET_EVENT_DATA:
            if (data->op_code == 1 && data->data_len > 0) {
                char *received = strndup(data->data_ptr, data->data_len);
                ESP_LOGI(TAG, "WebSocket Received: %s", received);

                char dotID[10];
                int x, y;
                if (sscanf(received, "%[^:]:%d,%d", dotID, &x, &y) == 3) {
