...
#define UART_NUM UART_NUM_1  // Using UART1 (You can change if needed)
#define BUF_SIZE 1024
#define TXD_PIN 17  // Change as per your ESP32 wiring
#define RXD_PIN 16  // Change as per your ESP32 wiring

static const char *TAG = "Stepper Motor";

// Structure to store the latest coordinates
typedef struct {
    int x;
    int y;
...
dot_coordinates_t latest_coordinates = {0, 0, false};

// Function to return the latest coordinates
dot_coordinates_t get_latest_coordinates() {
    latest_coordinates.updated = false;  // Reset flag after fetching
...
void move_to_position(int x_target, int y_target) 
{
    ...
    ESP_LOGI(TAG, "Moving to X: %d, Y: %d", x_target, y_target);
...
void uart_task(void *arg) {
    ...
    while (1) {
        ...
        if (len > 0) {
            ...
            // Parse data "D1:120,250"
            char dotID[10];
            int x, y;
            if (sscanf((char *)data, "%[^:]:%d,%d", dotID, &x, &y) == 3) {
                ESP_LOGI(TAG, "Parsed %s at X: %d, Y: %d", dotID, x, y);
                
                // Store in global struct
                latest_coordinates.x = x;
                latest_coordinates.y = y;
                latest_coordinates.updated = true; 
                ESP_LOGI(TAG, "dotID: %s, X: %d, Y: %d", dotID, x, y); // Mark as new data
