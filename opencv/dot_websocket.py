...
coordinate_queue = []  # Stores coordinates to be sent

# WebSocket server setup
async def send_coordinates(websocket):
    """Continuously send coordinates from queue to WebSocket clients."""
    while True:
        if coordinate_queue:
            data = coordinate_queue.pop(0)  # Get the next coordinate
            await websocket.send(json.dumps(data))  # Send as JSON
        await asyncio.sleep(0.1)  # Prevent busy waiting

async def start_websocket_server():
    """Starts the WebSocket server."""
    server = await websockets.serve(send_coordinates, "localhost", 9000)
...
def dot_recognition(image_path):
    ...
    coordinates = []  # Define the coordinates list here
    ...
    for i, kp in enumerate(keypoints, start=1):
        ...
        coordinate_queue.append({"id": i, "x": x, "y": y})
    ...
    coordinate_queue.extend(coordinates)
